...
export async function initOffendersPage() {
  ...
  const merged = topPlates.map((p) => ({
    plate: p.plate,
    count: p.count,
    lastSeen: p.last_seen,
    types: p.violations || [],
    risk: Math.min(99, 40 + (p.count || 0) * 8),
  }));

  const tbody = $("#offenders-tbody");
  if (tbody) {
    tbody.innerHTML = merged.length
      ? merged.slice(0, 10).map((o, i) => `
      ...
      : `<tr><td colspan="6">${emptyState("No repeat offenders", "Plates with multiple violations appear here.")}</td></tr>`;
